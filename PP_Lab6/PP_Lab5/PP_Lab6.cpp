...
DWORD WINAPI ThreadProc(LPVOID data)
{
    ...
    int thread_num = refined_data->thread_num;
    LogBuffer& log_buffer = refined_data->log_buffer;
    ...
    for (int y = line_start; y < line_end; y++)
    {
        for (int x = 0; x < image_width; x++)
        {
            ...
            *out.getPixel(x, y) = rgb32{ (uint8_t)avg_red, (uint8_t)avg_green, (uint8_t)avg_blue };
            ...
            fout << time_end - time_start << "\n";
            log_buffer.Add(time_end - time_start);
        }
    }
...
{
    if (InitializeCriticalSectionAndSpinCount(&CriticalSection, 0x00000400) == FALSE)
    {
        std::cout << "Critical section goes down\n";
    }
    
    HANDLE fileWriterTrigger = CreateEvent(NULL, TRUE, FALSE, TEXT("event"));
    IntQueue bufferData;
    LogBuffer logBuffer(bufferData, fileWriterTrigger);
    LogFileWriter logWriter(bufferData, "loggerData.txt", fileWriterTrigger);
    ...
    for (int iteration_of_blur = 0; iteration_of_blur < AMOUNT_OF_BLUR; iteration_of_blur++)
    {
        ...
        for (int i = 0; i < num_of_threads; i++)
        {
            ...
            LPVOID data = new ThreadData{ image, processed_image, fout[i], line_start, line_end, i, logBuffer};
        ...
        logWriter.Close();
        WaitForSingleObject(fileWriterTrigger, INFINITE);
